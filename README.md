...
meeting found ──→ NowPlayingShield holds the "Now Playing" role
                  → the media key is delivered to MeetMute instead of Music
...
Chromium build it (the same mechanism screen readers rely on). MeetMute sets it only while a
...
goes quiet. MeetMute combines three sources to avoid that:
...
```sh
make            # produces build/MeetMute.app
make run        # build and launch
make install    # copy to /Applications
make clean
```

A setup window appears on first launch. Grant the three permissions there and **restart MeetMute**.
...
| Developer ID / Apple Development | `identifier "io.github.genkitoyama.meetmute" and … certificate leaf[subject.OU] = <TeamID>` | **Preserved** |
